// Base Elementor template used for injection so users don't need to upload one
const DEFAULT_TEMPLATE = {
    ...
    "sections": {
        ...
        "highlights": [
            { "metric": "{{highlights.0.metric}}", "result": "{{highlights.0.result}}" }
        ],
...
document.getElementById("generateBtn").addEventListener("click", () => {
    ...
    try {
        const templateObj = JSON.parse(JSON.stringify(DEFAULT_TEMPLATE));
        const parsed = parseRawInput(rawArea.value);
        preview.value = JSON.stringify(parsed, null, 4);
        const resultObj = injectDataIntoTemplate(templateObj, parsed);
